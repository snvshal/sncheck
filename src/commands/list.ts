...
export async function listCommand(): Promise<void> {
  ...
  if (tasks.length === 0) {
    console.log(
      chalk.yellow('No tasks configured. Add a task with "sncheck add".')
    )
    return
  }
  ...
  const nameWidth = Math.min(
    22,
    Math.max(4, ...tasks.map((t) => t.name.length))
  )
  ...
  for (const task of tasks) {
    ...
    const description = truncate(task.description || "-", descWidth).padEnd(
      descWidth
    )
    console.log(
